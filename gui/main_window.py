...
class Demo(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.header_label = QLabel(
            "Bitcoin Wallet Generator", objectName="header_label"
        )
        ...
        self.generate_wallet_btn = QPushButton(
            "Generate Wallet", objectName="generate_wallet_btn"
        )
        ...
        priv_qr_container.addWidget(
            priv_qr_label, alignment=Qt.AlignmentFlag.AlignCenter
        )
        ...
        if ok and password:
            try:
                ...
                QMessageBox.information(
                    self,
                    "Export Successful",
                    f"Wallet exported successfully to {filename}",
                )
            except Exception as e:
                QMessageBox.critical(
                    self, "Export Error", f"Failed to export wallet: {str(e)}"
                )
    ...
    def export_wallet(self):
        if not hasattr(self, "current_wallet"):
            QMessageBox.warning(
                self,
                "Export Error",
                "No wallet to export. Please generate a wallet first.",
            )
            return

        filename, _ = QFileDialog.getSaveFileName(
            self, "Export Wallet", "", "Encrypted Wallet Files (*.ewf)"
        )
        if filename:
            password, ok = QInputDialog.getText(
                self,
            ...
            if ok and password:
                try:
                    export_encrypted_wallet(self.current_wallet, filename, password)
                    QMessageBox.information(
                        self, "Export Successful", "Wallet exported successfully."
                    )
                except Exception as e:
                    QMessageBox.critical(
                        self, "Export Error", f"Failed to export wallet: {str(e)}"
    ...
    def import_wallet(self):
        filename, _ = QFileDialog.getOpenFileName(
            self, "Import Wallet", "", "Encrypted Wallet Files (*.ewf)"
        )
        if filename:
            password, ok = QInputDialog.getText(
                self,
            ...
            if ok and password:
                try:
                    ...
                    set_qr_code(self.pub_qr_code, wallet.get("address", ""))
                    QMessageBox.information(
                        self, "Import Successful", "Wallet imported successfully."
                    )
                except Exception as e:
                    QMessageBox.critical(
                        self, "Import Error", f"Failed to import wallet: {str(e)}"
                    )
